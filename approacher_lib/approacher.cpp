...
// 非交互模式处理函数
double runNonInteractiveMode(const string& input_a, const string& input_b, bool quiet_mode, bool use_fuzzy_matching = false) {
    // 解析输入
    ...
    // 转换为特征列表
    auto features_a = parseFeatureList(parsed_a);
    auto features_b = parseFeatureList(parsed_b);
    ...
    if (quiet_mode) {
        ...
        // 构建显示字符串
        string display_a = "[";
        for (size_t i = 0; i < parsed_a.size(); i++) {
            if (i > 0) display_a += ",";
            display_a += parsed_a[i];
        }
        display_a += "]";

        string display_b = "[";
        for (size_t i = 0; i < parsed_b.size(); i++) {
            if (i > 0) display_b += ",";
            display_b += parsed_b[i];
        }
        display_b += "]";
...
void printHelp(const string& program_name) {
    ...
    cout << "  -f, --fuzzy     使用模糊匹配模式" << endl;
    cout << "\n示例:" << endl;
    cout << "  交互式模式: " << program_name << endl;
    cout << "  非交互模式: " << program_name << " \"red,apple\" \"green,apple\"" << endl;
    cout << "  静默模式:   " << program_name << " -q \"red,apple\" \"green,apple\"" << endl;
}
...
int main(int argc, char* argv[])
{
    ...
    if (!g_database->initialize("things/concepts-db")) {
        if (!quiet_mode) {
            cerr << "数据库初始化失败！" << endl;
        }
        return 1;
    ...
    // 加载测试数据
    if (!g_database->loadFromFile("things/example.txt", quiet_mode)) {
        if (!quiet_mode) {
            cerr << "加载测试数据失败！" << endl;
        }
        return 1;
    ...
    while (true) {
        ...
        if (line_a == "quit" || line_a == "exit") {
            ...
            if (g_database->saveParameters("things/parameters.txt")) {
                cout << "参数保存成功" << endl;
            }
            continue;
        } else if (line_a == "load") {
            if (g_database->loadParameters("things/parameters.txt")) {
                cout << "参数加载成功" << endl;
            }
            continue;
        }
