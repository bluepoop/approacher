...
private:
    ...
    string executeCommand(const string& command) {
        ...
        if (exit_code != 0) {
            throw runtime_error("命令执行失败 (退出码: " + to_string(exit_code) + ")");
        }
...
public:
     ...
     */
    ApproacherLibCaller(const string& lib_dir = "approacher_lib") : lib_path(lib_dir) {}
    ...
    double callApproacher(const string& input_a, const string& input_b,
        ...
        if (quiet_mode) command += " -q";
        if (use_fuzzy) command += " -f";
...
void runInteractiveTest(ApproacherLibCaller& caller) {
    ...
    while (true) {
        cout << "\n输入对象A: ";
        if (!getline(cin, input_a) || input_a == "quit") break;

        cout << "输入对象B: ";
        if (!getline(cin, input_b) || input_b == "quit") break;
        ...
        cout << "\n=== 语义增强分析器结果 ===" << endl;
        double semantic_score = caller.callSemanticApproacher(input_a, input_b, true);
...
int main() {
    ...
    for (size_t i = 0; i < test_cases.size(); i++) {
        ...
        cout << "\n[测试 " << (i+1) << "] \"" << test_case.first
             << "\" vs \"" << test_case.second << "\"" << endl;

        // 基础分析器测试
        double basic_score = caller.callApproacher(test_case.first, test_case.second);
        cout << "基础分析器: " << basic_score << endl;

        // 语义增强分析器测试
        double semantic_score = caller.callSemanticApproacher(test_case.first, test_case.second);
    ...
    string detailed_result = caller.getDetailedAnalysis("美丽,温柔,女孩", "女孩", true);
    cout << "\n详细分析结果:" << endl;
    ...
    cout << "执行 " << benchmark_rounds << " 次调用耗时: "
         << elapsed << " 秒" << endl;
    ...
    cout << "double score = caller.callApproacher(\"red,apple\", \"green,apple\");" << endl;
    cout << endl;
    cout << "// 语义增强调用" << endl;
    cout << "double semantic_score = caller.callSemanticApproacher(\"美丽,女孩\", \"女孩\");" << endl;
    cout << endl;
    cout << "// 详细分析" << endl;
    cout << "string analysis = caller.getDetailedAnalysis(\"input_a\", \"input_b\");" << endl;
